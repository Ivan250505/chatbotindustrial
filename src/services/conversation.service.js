...
class ConversationService {
    ...
    async processInfoCorreo(user, mensaje, telefono) {
        ...
        const sections = [{
            ...
            rows: [
                {
                    id: 'programa_0',
                    title: 'Tec. Produccion Ind.',
                    description: 'Tecnología en Producción Industrial'
                },
                {
                    id: 'programa_1',
    ...
    async processInfoPrograma(user, mensaje, telefono, messageType, interactiveId) {
        ...
        let programa = mensaje;
        if (messageType === 'interactive' && interactiveId) {
            const index = parseInt(interactiveId.replace('programa_', ''));
            if (!isNaN(index) && programas[index]) {
